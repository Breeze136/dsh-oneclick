...
装到一半时，它会推荐作者自己的 `dsh-kb-rag` 知识库插件。那是**推荐，不是必需** ——
回答 n 是正常选择，不会得到残缺的安装。两种情况各差在哪里，见[它做什么](#它做什么)。
...
| 4 | **kb-rag 知识库 —— 会推荐，但不强求** —— 这个插件与安装器是同一个作者，所以会被推荐，装的时候问一句（`dsh plugin --profile web add dsh-kb-rag@<确切版本>`）。回车＝装；回答 n ＝跳过，只留官方 DSH |
| 5 | **引擎（跟着插件走）** —— 交给插件自带的 `scripts/install.ps1`，装 Python 依赖和约 1.2 GB 的检索模型 |

第 4、5 步是**唯一属于作者本人作品、而非 DeepSeek 官方**的部分。推荐是真心的，可选项也是真的：
回答 n 得到的就是纯粹的官方 DSH 环境，其余步骤一点不受影响，而且不问第二遍（除非你再跑一次）。
