  ...
  if (-not $py -and $DryRun) {
    ...
    if ($NoModels) {
      ...
      if (Test-Path $rerankDir) {
        ...
        # v1.1.4：这里原本问一句「是否现在一并下载精排模型」。改成**不问、直接下载**。
        #
        # 为什么去掉这个提问：
        #   要答对这个问题，用户必须知道"精排是什么"、"它和基础模型差在哪"、
        #   "1.1GB 值不值"，而**最关键的后果——精排不可用时引擎不再判断
        #   「库里到底有没有这个内容」，问库外问题会返回看起来相关的内容——是引擎内部行为，
        #   用户没有任何观测手段**。所以这不是一个"选择"，是让用户替我们决定一个
        #   他无从评估的内部实现，而且答错的代价是**静默的**（没有报错，只有"好像不太准"）。
        #
        # 判据（可推广到其他提问）：只有"用户有能力评估后果"的选择才配出现在安装流程里。
        #   成本是时间/流量这种用户会算的东西 → 可以说，但要给结论；
        #   成本是内部行为/精度这种用户算不出来的东西 → 不问，默认按正确的来。
        #   精排恰好两者都有：1.1GB 用户能理解，但"失去无关判定"他理解不了 → 归到"不问"。
        #
        # 仍然可以不要：-NoModels（命令行参数）是显式的便宜出口，留给知道自己在做什么的人。
        #   只是它不再是一个会拦住所有人的提问。
        Write-Host ''
        Say '现在下载检索模型（这一步是自动的，不需要你选）：'
        Say '   基础检索模型 95MB + 精排模型约 1.1GB。'
        # 用"用户能评估的成本"说话：不解释算法，只说多花多少时间、少等在哪。
        Say '   精排模型的作用是让引擎能判断「库里到底有没有这个内容」：'
        Say '   没有它，问库外的问题也会返回看起来相关的结果。所以默认装上。'
        Say '   （慢网会多花几分钟；不想现在下可以重新运行本脚本并加 -NoModels）'
